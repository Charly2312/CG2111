...
#define DEBOPUNCE_INTERVAL 100 // dobounce time: unit: ms
...
{
	int quit=0;
	bool wasPressed = false; // trace if wsad is pressed
	char lastkey = 0; // last key pressed
	clock_t lastTime = 0; // last pressing time
	printf("Command (w=forward, s=reverse, a=turn left, d=turn right, p=clear stats, c=get stats q=exit)\n");
	while(!quit)
	{
		char ch;
	//	printf("Command (w=forward, s=reverse, a=turn left, d=turn right, e=stop, p=clear stats, c=get stats q=exit)\n");
		...
		if (kbhit()) {
			...
			case 's':
			case 'S':
				if (!wasPressed || (currentTime - lastTime) >= DEBOPUNCE_INTERVAL) {
					params[0] = 50;
					params[1] = 50;
					buffer[1] = ch;
			...
			// if the last key is one of WASD, and pass 100ms, then send stop() instruction
			if (lastKey == 'w' || lastKey == 'W' || lastKey == 's' || lastKey == 'S' || lastKey == 'a' || lastKey == 'A' || lastKey == 'd' || lastKey == 'D') {
				if ((clock() - lastTime) >= DEBOPUNCE_INTERVAL) {
					ch = 'e';
					params[0] = 0;
					params[1] = 0;
					memcpy(&buffer[2], params, sizeof(params));
