...
// debounce time
#define DEBOUNCE_DELAY_US 200000 // 0.2 seconds = 200,000ms
...
void *writerThread(void *conn)
{
	...
	auto debounceStartTime = std::chrono::steady_clock::now();
	...
	//	printf("Command (w=forward, s=reverse, a=turn left, d=turn right, e=stop, p=clear stats, c=get stats q=exit)\n");
		...
		if (kbhit()) {
			...
			case 'D':
				...
				if (!keyPressed || elaspsed > 200) { // if key is not pressed before or pressing time exceeds 0.2s
					keyPressed = true;
					params[0] = 50;
					params[1] = 50;
					buffer[1] = ch;
