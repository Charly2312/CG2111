...
// From Ultrasonic_sensor.h
Ultra ultra(ECHO, TRIG);
// From color_sensor.h
Cs cs(S0, S1, S2, S3, sensorOut, redMin, redMax, greenMin, greenMax, blueMin, blueMax);
...
// Ultrasonic sensor pins wiring to Arduino
...
void sendStatus() {
  // Take 5s to run because of Cs.colorSense(); 
  ...
  Cs.colorSense();
  statusPacket.params[10] = Cs.get_color();
  statusPacket.params[11] = Cs.get_redValue();
  statusPacket.params[12] = Cs.get_blueValue();
  statusPacket.params[13] = Cs.get_greenValue();

  // store the distance from the closest object in front to prevent collision
  statusPacket.params[14] = Ultra.getDistance();
  // send packet
  sendResponse(&statusPacket);
}


// Do not have color information, faster than sendStatus()
void sendDist() {
  TPacket statusPacket;
  statusPacket.packetType = PACKET_TYPE_RESPONSE;
