...
public:
  ...
  void play_track(uint32_t file);            // Play selected track
...
private:
  ...
  enum { CMD_BUF_LEN = 10 };  // enum: a special type that represent a list of constants.  enum Level {LOW, MEDIUM,HIGH}; Be dafult LOW = 1 MEDIUM = 2 HIGH = 3(increment)
